...
public class DefaultInitializationTest()
{
    ...
    {
        var interfaceType = typeof(IDatabaseSyncableDefault<,>);
    ...
    public async Task TestDefaultInitialization(string syncableClassName)
    {
        ...
        var defaultCreateMethod = syncableClass.GetMethod("CreateDefault", BindingFlags.Static | BindingFlags.Public);
        var key = CreateArgument(defaultCreateMethod.GetParameters().First().ParameterType);
        var obj1 = defaultCreateMethod.Invoke(null, new object[] { key });
