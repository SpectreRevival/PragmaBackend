CREATE TABLE IF NOT EXISTS gamepad_config (
    ...
    CONSTRAINT verify_custom_look_config CHECK (
        ...
        AND (custom_look_config).look_settings.yaw_rate IS NOT NULL
        AND (custom_look_config).look_settings.pitch_scale IS NOT NULL
        AND (custom_look_config).look_settings.turn_accel_yaw_bonus IS NOT NULL
        AND (
            custom_look_config
        ).look_settings.turn_accel_pitch_bonus IS NOT NULL
        AND (
            custom_look_config
        ).look_settings.turn_accel_delay_seconds IS NOT NULL
        AND (
            custom_look_config
        ).look_settings.turn_accel_time_to_max IS NOT NULL
        AND (custom_look_config).look_settings_ads.yaw_rate IS NOT NULL
        AND (custom_look_config).look_settings_ads.pitch_scale IS NOT NULL
        AND (
            custom_look_config
        ).look_settings_ads.turn_accel_yaw_bonus IS NOT NULL
        AND (
            custom_look_config
        ).look_settings_ads.turn_accel_pitch_bonus IS NOT NULL
        AND (
            custom_look_config
        ).look_settings_ads.turn_accel_delay_seconds IS NOT NULL
        AND (
            custom_look_config
        ).look_settings_ads.turn_accel_time_to_max IS NOT NULL
    ),
    CONSTRAINT verify_custom_response_curve CHECK (
        (custom_response_curve).display_name IS NOT NULL
        AND (custom_response_curve).exponent IS NOT NULL
        AND (custom_response_curve).response_curve_arc_degree IS NOT NULL
        AND (custom_response_curve).response_curve_slope IS NOT NULL
        AND (
            custom_response_curve
        ).response_curve_linear_blend_power IS NOT NULL
    )
);
