CREATE TABLE IF NOT EXISTS color_vision_config (
	...
	CONSTRAINT verify_outline_color CHECK (
		(outline_color).r IS NOT NULL AND
		(outline_color).g IS NOT NULL AND
		(outline_color).b IS NOT NULL AND
		(outline_color).a IS NOT NULL
	),
	CONSTRAINT verify_outline_color_lower CHECK (
		(outline_color_lower).r IS NOT NULL AND
		(outline_color_lower).g IS NOT NULL AND
		(outline_color_lower).b IS NOT NULL AND
		(outline_color_lower).a IS NOT NULL
	)
);